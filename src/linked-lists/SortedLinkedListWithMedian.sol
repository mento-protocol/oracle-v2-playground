...
// TODO: If we continue using this, re-enable and address solhint findings
// solhint-disable
pragma solidity ^0.8.24;

import {LinkedList} from "./LinkedList.sol";
import {SortedLinkedList} from "./SortedLinkedList.sol";
...
library SortedLinkedListWithMedian {
    ...
    function push(List storage list, bytes32 key) internal {
        insert({
            ...
            value: 0,
            lesserKey: bytes32(0),
            greaterKey: list.list.list.tail
        });
    }
     ...
     * @return keys The keys of the popped elements.
     */
    function popN(
        List storage list,
        uint256 n
    ) internal returns (bytes32[] memory keys) {
        // solhint-disable-next-line custom-errors, gas-strict-inequalities
    ...
    function updateMedian(List storage list, MedianAction action) private {
        LinkedList.Element storage previousMedian = list.list.list.elements[
            list.median
        ];
